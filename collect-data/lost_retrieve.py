...
def clear(temp):
    ...
    temp = re.sub(r'\bείχα\b|\bείχες\b|\bείχε\b|\bείχαμε\b|\bείχατε\b|\bείχαν\b|\bείχανε\b|\bέχε\b|\bέχοντας\b', ' ',
                  temp)
            ...
            if "Ενεστώτας" in i.get_text().split() and "Μέση-Παθητική" not in i.get_text().split():
                ...
                try:
                    if "Ενεστώτας" in soup.tbody.find_next(
                            ...
                            'tbody').get_text().split():  # and "Ευκτική" not in soup.tbody.find_next('tbody').get_text().split():
                        temp = re.sub(r'[^α-ωΑ-Ωά-ώΆ-Ώΐΰ]', ' ', temp)
                        clear(temp)
                        for j in set(temp.split()):
                            if j != title:
